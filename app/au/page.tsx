...
function Spark({
  ...
  if (data.length < 2) {
    return <div className="text-sm opacity-60">sin datos…</div>;
  }
  ...
  const d = xs
    .map(
      (x, i) =>
        `${i === 0 ? "M" : "L"} ${x.toFixed(2)} ${ys[i].toFixed(2)}`
    )
    .join(" ");
...
export default function AUDashboardPage() {
  ...
  // URL relativa (sin window) → compatible Vercel/build
  const url = useMemo(() => {
    ...
    return `/api/au/stream?${params.toString()}`;
  }, [intent, dd, pg, cc, ms]);
  ...
  useEffect(() => {
    ...
    es.onerror = () => {
      // cerramos para evitar loops raros si cae la conexión
      es.close();
      esRef.current = null;
    };
          ...
          {!latest ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
              <div className="rounded-xl p-3 bg-black/30">
                Ψ<br />
                <b>{latest.Psi.toFixed(3)}</b>
              </div>
              <div className="rounded-xl p-3 bg-black/30">
                R<br />
                <b>{latest.R.toFixed(3)}</b>
              </div>
              <div className="rounded-xl p-3 bg-black/30">
                T<br />
                <b>{latest.T.toFixed(3)}</b>
              </div>
              <div className="rounded-xl p-3 bg-black/30">
                P<br />
                <b>{latest.P.toFixed(3)}</b>
              </div>
              <div className="rounded-xl p-3 bg-black/30">
                Entropía r<br />
                <b>{latest.Er.toFixed(3)}</b>
              </div>
              <div className="rounded-xl p-3 bg-black/30">
                Fase<br />
                <b>{latest.phase.toFixed(3)}</b>
              </div>
              <div className="rounded-xl p-3 bg-black/30">
                Dominance<br />
                <b>
                  {latest.dom?.whoDominates}/{latest.dom?.channel}
                </b>
              </div>
              <div className="rounded-xl p-3 bg-black/30">
                Rotación<br />
...
<div className="rounded-xl p-3 bg-black/30 md:col-span-2">
  ...
  <b className="break-all">{latest.modeReason ?? "—"}</b>
              ...
              <div className="rounded-xl p-3 bg-black/30 md:col-span-2">
                Hash
                <br />
                <b className="break-all">{latest.hash}</b>
              </div>

              <div className="rounded-xl p-3 bg-black/30 md:col-span-2">
                M4
                <br />
                <b className="break-all">{JSON.stringify(latest.m4)}</b>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="text-xs opacity-60">
        Abre esta página y mueve sliders: el motor reacciona en vivo. URL stream:{" "}
